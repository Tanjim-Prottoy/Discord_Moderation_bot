...
module.exports = {
    ...
    run: async (client, message, args) => {
      ...
      const embed3 = new MessageEmbed()
        .setDescription(`${config.emojis.wrong} This is a voice channel, not a text channel.`)
        .setColor(config.messages.embeds.colors.no);

      if(!channel.isText()) return message.reply({ embeds: [embed3] })

      const embed4 = new MessageEmbed()
        .setDescription(`${config.emojis.wrong} Please mention a role.`)
        .setColor(config.messages.embeds.colors.no);

      if(!args[1]) return message.reply({ embeds: [embed4] });
      ...
      if(!role) return message.reply({ embeds: [embed5] });
